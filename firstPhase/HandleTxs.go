...
func (h *HandleTxs) Handler(possibleTxs []Transaction) []Transaction {
	...
	for _, transaction := range possibleTxs {
		...
		if isValid {
			...
			// fmt.Println("removujem tx")
			// fmt.Println(transaction)
			h.removeUTXO(transaction)
		}
	}
	return validTransactions
}

func (h *HandleTxs) removeUTXO(tx Transaction) {
	for _, output := range tx.Outputs {
		for utxo, utxoOutput := range h.UTXOPool.H {
			if utxoOutput.address == output.address && utxoOutput.value == output.value {
				h.UTXOPool.RemoveUTXO(utxo)
				fmt.Println(h.UTXOPool.H)
			}
		}
	}
}
